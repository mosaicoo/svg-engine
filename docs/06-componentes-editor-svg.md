...
| `<svge-effects-panel>`              | **D-047 + D-144 (2026-06-29)** Editor do pipeline de efeitos. Picker por categoria (adiciona ao pipeline), reordenação/remover por estágio, **chain** (≥2 efeitos). **D-144**: por estágio renderiza controles nativos dos `Effect.params` (slider+number / cor / select / checkbox), chips de `presets` e Reset. Aplica via re-encode do `style.filter` + `SetStylePropertyOnManyCommand` (1 undo); storage stateless (`url(#effectId)` / `url(#svge-chain-…)` / `url(#svge-fx-…)`). Só `MatIcon`/`MatIconButton`                                                                                                           | 6    |
