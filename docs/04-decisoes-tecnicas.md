...
- **Decisão (minificação): NÃO — TENTADA E REVERTIDA (NPM-FIX, v0.1.1)**. O Angular Package Format **não minifica** de propósito: o FESM publicado vem em **partial compilation** (declarações `ɵɵngDeclare…`) e o **Angular Linker** do build do consumidor as compila para AOT. Tentamos minificar com esbuild (`dist:prepare` chamando `minify:fesm`) para um pacote menor, mas isso **quebrou o consumo**: o esbuild escapa o caractere `ɵ` (U+0275) para `ɵ`, e o Linker detecta o que processar procurando o `ɵ` **literal** — escapado, ele pula a lib; em runtime o Angular tenta **JIT** e falha (`'_vt' needs to be compiled using the JIT compiler`, pois `@angular/compiler` não está no bundle de produção). A v0.1.0 saiu minificada/quebrada → revertido o minify, removido `scripts/minify-fesm.mjs`, `dist:prepare` = `build:lib` → `strip:maps`, e republicado como **0.1.1**. **Lição**: validar lib publicada em **runtime** (não só `build passa`); o tamanho do FESM (1,4 MB tarball) é irrelevante — o **consumidor** minifica no build dele (bundle final pequeno).
